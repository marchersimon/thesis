...
When the voltage in the secondary coil is high enough to ionize the air around it, a spark forms. This spark then discharges the coil and extinguishes as soon as the voltage drops to a low level. The key component to model this behaviour in LTSpice is a voltage controlled switch. It is an ideal, lossless component which simply connects or disconnects its two terminals depending on the voltage difference of its \texttt{+} and \texttt{-} pin. For more specific use cases, the switch can be used with an hysteresis.

To specify a switch's characteristics, a new switch model has to be defined. For the example of a switch with a hysteresis, this would be \cinl{.model model_name SW(Vt = 20 Vh = 5)}. This switch has a threshold voltage of 20\,V and a hysteresis voltage of 5\,V, which means that it starts conducting above 25\,V and stops conducting below 15\,V. \(R_{ON}\) defaults to 1\,\(\Omega\), \(R_{OFF}\) to \(1\,T\Omega\), and all other parasitics are disabled by default.

This makes it possible to short the secondary coil as soon as its voltage reaches a certain threshold and opening the short when it falls below another threshold. However, the voltage in the secondary coil has no DC component and therefore two zero-crossings every period, so the turn-off threshold would be reached every cycle.% rewrite this sentence
To have the switch only turn off after it full discharged, the average voltage has to be used instead of the momentary voltage. Also, since the voltage is symmetrical around 0\,V, it has to be rectified first. The averaging part can be modeled with a tiny capacitor and the rectifying can be done by a single diode\sidenote{Known as halve-wave rectifier}. To then discharge the capacitor itself, another switch has to be used.
...
Because of the rudimentary power engineering expertise this project was tried to be built upon, some of the planned design stages did not go as hoped. The best example for this is this amplifier. Deriving equations for a class-E amplifier with a tesla resonator as load goes far beyond this education's curriculum. 
