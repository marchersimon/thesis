...
By modulating another frequency onto this carrier frequency, a.k.a. turning the coil on and off at this rate, the frequency is also modulated onto the sound wave created by the arc. Since the tesla coil has to complete at least one cycle before being turned off, the modulated frequency cannot be greater than the carrier frequency. The figure below shows this process in more detail. The first diagram shows the idealized displacement of the air, while the second diagram takes the inertia of the air molecules into account. The third diagram depicts the frequency components of this sound wave and shows, that both the carrier frequency and the modulated frequency are present. 
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot+[draw=black, fill=black!5] table [x=t, y=rect, col sep=comma, mark=none]{simon/resources/signal.csv};
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot+[draw=black, fill=black!5] table [x=t, y=disp, col sep=comma, mark=none]{simon/resources/signal.csv};
  \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot+[ycomb, draw=black, mark options={black!50}] table [x=f, y=mag, col sep=comma]{simon/resources/fourier.csv};
  \end{axis}
\end{tikzpicture}
