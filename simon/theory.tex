...
This topology uses a class E RF amplifier to produce the necessary power for driving the primary coil. By utilizing a technique called ZVS and ZCS\sidenote{\underline{Z}ero \underline{v}oltage \underline{s}witching and \underline{z}ero \underline{c}urrent \underline{s}witching}, the dissipated power in the switching device is drastically reduced, making this type of amplifier nearly 100\% efficient in theory. A class E amplifier is capable of switching very high frequencies, which makes it an ideal driver for smaller tesla coils operating at a few MHz.
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot+[draw=black, fill=black!10] table [x=t, y=rect, col sep=comma, mark=none]{simon/resources/signal.csv};
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot+[draw=black, fill=black!10] table [x=t, y=disp, col sep=comma, mark=none]{simon/resources/signal.csv};
  \end{axis}
\end{tikzpicture}
