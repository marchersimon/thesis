...
After designing the \gls{pcb}'s layout, it has to be exposed before it can be etched. The 

\todo{entwickeln oda ätzen wegen photoresist}. This way only the copper parts that are not hit with UV light will be protected and the rest will dissolve in the etching phase. 

To be able to etch, the \gls{pcb} first has to be exposed

\subsection{Stripping and Etching}


The copper also tended to not dissolve in certain places often resulting in an unwanted short. 
