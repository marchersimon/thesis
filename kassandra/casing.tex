...
It might seem like the casing is not very significant, but the design and function of all kinds of devices' exteriors are crucial. Even the simplest casings often have multiple design stages and variations with different advantages. Since the Tesla coil is intended as a consumer product, the casing plays an essential role in its appearance. however, apart from optics, there are many safety hazards, which the casing has to account for. Most notably is the very real fire risk, which goes along with playing with open plasma.
...
Deciding which materials to use is also a tedious task because there are a lot of details and possibilities that have to be considered when designing a Tesla coil. Many factors determine the best working material, most affecting the coils directly and therefore being unsuitable, but others are scrapped because of their low availability or costly construction. The two main criteria that decided whether or not it was used, those being who it reacts to high temperatures and if it interferes with the coils circuit e.g. disrupting its magnetic field. Another factor would be the materials costs and how hard it would to process it. The materials appearance was more negligible, but in some cases it was graded. 
...
Small, complex structures like the primary supports are best manufactured with an automated process like 3D printing. This makes it possible to construct both the supports and the platform their mounted on in one piece, erasing the need for bolts entirely. The way 3D printing works is by drawing multiple layers of fine plastic lines, which enables it to easily construct all kind of pieces with little effort. But it also makes the end product inaccurate, which is unsuitable for the small half circles on the supports, as they will most likely end up too small for the wire of the primary coil. 

Another way to manufacture the supports would be to cut a plastic plane with a laser. This laser cutting method is of course only possible to construct 2D components, which means the supports and the platform had to be made separately. On the other side, laser cutting is a relatively accurate, ensuring that the slots for the primary and also other parts like the bolts fit with only minor post processing. 
...
Given this device's peculiar shape, that has to be very accurate\sidenote{when it comes to shape} the best way to manufacture would be 3D printing. Even though the grooving was made bigger the wire was still hard to press into the bending device, because of the way a 3D printer works. In the end the bent wire of the primary still had to be glued to the supports so it would not fall off. 
